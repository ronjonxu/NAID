...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--patch_sizeH', type=int, default=150)
        parser.add_argument('--patch_sizeW', type=int, default=100)
        parser.add_argument('--mask_size', type=int, default=1)
        ...
        # Scheduler
        # MultiStepLR
        parser.add_argument('--gamma_MSLR', type=float, default=0.1)
        parser.add_argument('--milestones', type=str, default='400',
                help='Separate the milestones by `,`, using 400 by default. ')
        # CosineAnnealingLR
    ...
    def parse(self):
        ...
        # process milestone
            p = re.compile('[^-0-9]+')
            milestones = [int(i) for i in re.split(p, opt.milestones) if int(i) >= 0]
